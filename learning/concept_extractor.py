...
class LLMClient:
    def __init__(self, api_base: str, temperature: float = 0.2, request_timeout: float = 120.0, retries: int = 2):
...
SYSTEM_PROMPT = """You extract formal knowledge from textbooks into a compact graph.
Return STRICT JSON ONLY that matches the schema. Do NOT include notes, prose, or explanations. DO NOT include markdown.
  ...
  "concepts": [
    {
      ...
      "formal": "<LaTeX or plain formula if applicable, else null>",
...
Rules:
- Prefer concise, canonical names ("Product Rule", "Chain Rule", "Limit").
- Put worked I/O pairs into examples when present; otherwise leave empty.
- Only include high-signal dependencies needed to learn/solve problems.
- If nothing meaningful is present, return {"concepts": [], "edges": []}.

Remember, you MUST ONLY output JSON using the provided schema. DO NOT output anything other than valid JSON.
"""

USER_PROMPT_TEMPLATE = """Extract concepts and dependencies from this textbook slice.
Remember to follow the provided JSON schema within the system prompt exactly. Do not include any additional explanations or context. ONLY output JSON.
